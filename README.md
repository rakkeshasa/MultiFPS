...
만약 먼저 움직였던 클라이언트 캐릭터가 정상적인 위치에 있지 않다면 해당 위치를 서버 환경에 맞춰야 한다. 이것을 동기화 문제라 한다.</br>
따라서 클라이언트는 서버로부터 얼마나 많은 요청이 갔는지 파악하고 예측을 해야하고 동기화 문제가 발생한 경우에는 수정을 해야한다.</br>
